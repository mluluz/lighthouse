...
impl Attesters {
    fn add(&mut self, additional_indices: &HashSet<usize>, additional_balance: u64) {
...
impl GroupedAttesters {
    pub fn new(state: &BeaconState, spec: &ChainSpec) -> Result<Self, BeaconStateError> {
        ...
        for a in &state.latest_attestations {
            ...
            let attesting_balance = state.get_total_balance(attesting_indices.iter(), spec);
...
fn is_from_epoch(a: &PendingAttestation, epoch: Epoch, spec: &ChainSpec) -> bool {
    a.data.slot.epoch(spec.epoch_length) == epoch
}
...
fn has_common_epoch_boundary_root(
    ...
    let slot = epoch.start_slot(spec.epoch_length);
